...
import { useAuditLogs } from "@/lib/clientforge/audit";
import { getAllDynamicLeads } from "@/lib/clientforge/leads-store";
import { getAllDynamicDrafts } from "@/lib/clientforge/drafts-store";
import { getAllDynamicDocuments } from "@/lib/clientforge/documents-store";
import { formatCurrency, formatDateTime } from "@/lib/clientforge/formatters";
...
export default function DashboardPage() {
  const dynamicLogs = useAuditLogs();
  ...
  const liveLeads = [...getAllDynamicLeads(), ...seedLeads];
  const liveDrafts = [...seedDrafts, ...getAllDynamicDrafts()];
  const liveDocuments = [...seedDocuments, ...getAllDynamicDocuments()];
  const stats = {
    totalLeads: liveLeads.length,
    hotLeads: liveLeads.filter((l) => l.score === "hot").length,
    docsIndexed: liveDocuments.filter((d) => d.status === "indexed").length,
    draftsReady: liveDrafts.filter((d) => d.status !== "sent").length,
    pipelineValue: liveLeads.reduce((sum, lead) => sum + lead.value, 0),
  };
  const recentLeads = liveLeads.slice(0, 4);
