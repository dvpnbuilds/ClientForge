...
import { logAction, getLogsForLead } from "@/lib/clientforge/audit";
import { formatCurrency, formatDateLong } from "@/lib/clientforge/formatters";
import type { Lead, LeadScoreResult, DraftProposal, ActionLog } from "@/lib/clientforge/types";
...
function LeadDetailContent({
  ...
  const [leadLogs, setLeadLogs] = useState<ActionLog[]>(() => getLogsForLead(lead.id));
  ...
  const generate = useCallback(
    (type: "reply" | "proposal" | "both") => {
      ...
      setTimeout(() => {
        ...
        setLeadLogs(getLogsForLead(lead.id));
        setGenerating(null);
      }, delay);
    },
    [lead, scoreResult]
